...
Adafruit_NeoPixel::Adafruit_NeoPixel(uint16_t n, uint8_t p, uint8_t t) : numLEDs(n), numBytes(n * 3), pin(p), type(t), pixels(NULL), inpin(0)
#ifdef AVR
...
void Adafruit_NeoPixel::resize(uint16_t n) {
  free( pixels );
  numLEDs = n;
  numBytes = numLEDs * 3;
  if((pixels = (uint8_t *)malloc(numBytes))) {
...
void Adafruit_NeoPixel::setinpin(uint8_t p) {
  inpin = p;
}
